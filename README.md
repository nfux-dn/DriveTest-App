...
**Providers:** OpenAI, Anthropic, and **Cursor** (via the `cursor-agent` CLI, run headlessly
in read-only `ask` mode — the key is a Cursor user/service API key). Cursor runs a one-shot
Composer agent and the platform parses its answer as the verdict.
