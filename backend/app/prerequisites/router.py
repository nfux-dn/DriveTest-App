"""Prerequisite routes (spec section 29)."""
...
@router.get(
    "/suites/{suite_id}/environments/{environment_id}/prerequisites",
    response_model=PrerequisiteTemplate,
)
def get_prerequisites(
    ...
    return service.resolve_for(db, suite_id, environment_id)
...
def validate_prerequisites(
    ...
    return service.validate(db, payload.suite_id, payload.environment_id, payload.values)
...
def run_check(
    ...
    return service.run_field_check(
        db, payload.suite_id, payload.environment_id, field_id, payload.values
    )
